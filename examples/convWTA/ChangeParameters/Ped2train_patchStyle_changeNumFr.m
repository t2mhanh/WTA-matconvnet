...
clear all
clc
run('/nobackup/schtmt/matconvnet-1.0-beta24/matlab/vl_setupnn.m')
addpath('~/libsvm-3.22/matlab')

stride = 12;
mag_thres = 50;
patch_size = 24;
pool_size = patch_size; % change 'pool_size' will change 'dim'
dim = 128;
Nfr = 9
...
savePath = ['./data/Ped2_5frame_27epoch_patchStyle_pa' num2str(patch_size) '_str' num2str(stride) '_magthres' num2str(mag_thres) '_dim' num2str(dim) '_Nfr' num2str(Nfr) '/localSVM'];
...
%
 ...
 for trainseq = 1:16
    ...
    GlobalData_local = [];
    for fr = 1:nfr-Nfr + 1
        curfl = Flow(:,:,:,fr:fr+Nfr-1);
      	if Nfr > 1
            mag = sum(curfl(:,:,:,ceil(Nfr/2)).^2,3);
      	else
      	    mag = sum(curfl.^2,3);
      	end
