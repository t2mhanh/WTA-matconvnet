...
savePath = ['./data_1/Ped1_5frame_27epoch_pa' num2str(patch_size) '_str' num2str(stride) '_magthres' num2str(mag_thres_train) '/localSVM'];
savePath1 = fullfile(savePath,'Grid5x8');
if ~exist(savePath1,'dir'); mkdir(savePath1); end

%% SVM model
load(fullfile(savePath,'localSVMGrid5x8_GlobalMaxMin.mat')) % 'localSVM' minimums ranges
    ...
    for fr_n = 1:nfr-4
        ...
        for r = 1:size(feat,1)
            for c = 1:size(feat,2)
                ...
                curmodel = localSVM{min(ceil(r/4),size(localSVM,1)),min(ceil(c/4),size(localSVM,2))};
