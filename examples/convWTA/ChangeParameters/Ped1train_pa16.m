...
%	size(TrainFeat)
 ...
 save(fullfile(savePath,'LocalData_Grid21x33.mat'),'dataGrid','minimums','ranges','-v7.3')
 save(fullfile(savePath,'GlobalData_Grid1x1.mat'),'GlobalData','minimums','ranges','-v7.3')

 %% train local SVM Grid 12x18
% load(fullfile(savePath,'LocalData_Grid21x33.mat'))%dataGrid, minimums, ranges
% disp('Train SVM')
% % localSVM = cell(12,18);
...
for i = 1:size(data,1)
    for j = 1:size(data,2)
        r = min(ceil(i/4),size(dataGrid,1));
        c = min(ceil(j/4),size(dataGrid,2));
        if length(dataGrid{r,c}) == 0
...
save(fullfile(savePath,'LocalData_Grid5x8.mat'),'dataGrid','minimums','ranges','-v7.3')
%% train local SVM
% load(fullfile(savePath,'LocalData_Grid5x8.mat'))%dataGrid, minimums, ranges
disp('Train SVM')
localSVM = cell(5,8);
nu = 2^-9;
gamma = 2^-7;
...
save(fullfile(savePath,'localSVMGrid5x8_GlobalMaxMin.mat'),'localSVM','minimums','ranges')
