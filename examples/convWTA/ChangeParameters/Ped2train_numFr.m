...
savePath = ['./data_1/Ped2_5frame_27epoch_pa' num2str(patch_size) '_str' num2str(stride) '_magthres' num2str(mag_thres) '_Nfr' num2str(Nfr) '/localSVM'];
...
%        mag = sum(curfl(:,:,:,3).^2,3);
 ...
 %       TrainFeat = mean(gather(net.vars(predVar).value),4);
	if Nfr > 1
            ...
            TrainFeat = gather(net.vars(predVar).value);
        end
...
% load(fullfile(savePath,'GlobalData_Grid1x1.mat'))%GlobalData, minimums, ranges
 disp('Train SVM')
 nu = 2^-9;
 gamma = 2^-7;
 param = ['-q -s 2 -n ', num2str(nu), ' -g ', num2str(gamma)];
