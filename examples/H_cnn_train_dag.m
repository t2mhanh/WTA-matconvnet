function [net,stats] = H_cnn_train_dag(net, imdb, getBatch, varargin)
...
%     inputs = params.getBatch(params.imdb, batch) ;
    inputs = params.getBatch(params.imdb, batch) ; %HANH modify
...
%       params.getBatch(params.imdb, nextBatch) ;
      params.getBatch(params.imdb, nextBatch) ; %HANH modify
    end
...
% Hanh added for showing error from other objective functions
sel = [find(cellfun(@(x) isa(x,'dagnn.Loss'), {net.layers.block})) find(cellfun(@(x) isa(x,'dagnn.Pdist'), {net.layers.block})) ...
    find(cellfun(@(x) isa(x,'dagnn.binaryLoss'), {net.layers.block})) find(cellfun(@(x) isa(x,'dagnn.binaryEntropyLoss'), {net.layers.block})) ...
    find(cellfun(@(x) isa(x,'dagnn.EuclideanLoss'), {net.layers.block})) find(cellfun(@(x) isa(x,'dagnn.EuclideanLossOpticalWeight'), {net.layers.block}))];
for i = 1:numel(sel)
